...
from MTG import card as _cardmod
...
def add_hand_ability(cardname, mana_cost, effect, target_specs=None):
    """Register a from-hand activated ability (cycling, bloodrush, ...).

    mana_cost: a plain mana string ("2", "1B", ...). The card is discarded as
    part of the cost (handled by player.get_action's 'c' branch).
    effect: code string, eval'd with `self` bound to the card.
    target_specs: list of target-criteria strings, or None.
    """
    if not name_to_id(cardname):
        return
    card = card_from_name(cardname, get_instance=False)
    if card.hand_abilities is _cardmod.Card.hand_abilities:  # still shared class default
        card.hand_abilities = []
    if target_specs:
        target_specs = utils.parse_targets(list(target_specs))
    card.hand_abilities.append((mana_cost, effect, target_specs))


def apply_collection_hand_abilities():
    """Load data/collection_hand_abilities.py (generated by translate_oracle)."""
    try:
        from data import collection_hand_abilities as _h
    except Exception:
        return
    for entry in getattr(_h, "HAND_ABILITIES", []):
        add_hand_ability(*entry)
...
def setup_cards(FILES=['data/m15_cards.txt', 'data/cube_cards.txt',
                       'data/collection_cards.txt'], force=False):
    """
    Read in cards information from data/cards.txt

    Logs in setup_cards.log

    Idempotent: the ability builders (add_activated_ability / add_trigger /
    add_static_effect / ...) append to *class-level* lists, so calling this
    twice would stack a second copy of every ability. Guarded with a flag;
    pass force=True to re-run deliberately (e.g. after changing FILES).
    """
    global _SETUP_DONE
    ...
    apply_collection_hand_abilities()